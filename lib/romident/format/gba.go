...
func ParseGBA(r io.ReaderAt, size int64) (*GBAInfo, error) {
	...
	title := extractASCII(header[gbaTitleOffset : gbaTitleOffset+gbaTitleLen])

	// Extract game code
	gameCode := extractASCII(header[gbaGameCodeOffset : gbaGameCodeOffset+gbaGameCodeLen])

	// Extract maker code
	makerCode := extractASCII(header[gbaMakerOffset : gbaMakerOffset+gbaMakerLen])
