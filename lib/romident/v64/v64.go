...
// Identify verifies the format and extracts game identification from a V64 ROM.
func Identify(r io.ReaderAt, size int64) (*core.GameIdent, error) {
	...
	if actualOrder != z64.N64ByteSwapped {
		return nil, fmt.Errorf("byte order mismatch: expected v64, got %s", actualOrder)
	}
	...
	z64.SwapBytes16(header)

	info, err := z64.ParseN64Header(header, z64.N64ByteSwapped)
	if err != nil {
		return nil, err
	}
